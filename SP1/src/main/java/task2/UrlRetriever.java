...
public class UrlRetriever extends Thread {
    ...
    UrlRetriever t = new UrlRetriever(new URL("https://da.wikipedia.org/wiki/Costa_Coffee"));
    t.start();
    t.join();
    System.out.println("data: "+t.getData().length);  
    
    }
  
      private URL url;
     private  byte[] data;
    public UrlRetriever (URL aurl){
    ...
    public void run() {
        ...
        System.out.println("Run for "+this.getName()+" done ");
    }
     ...
     */
    public byte[] getData() throws IllegalStateException, IOException {
        return data;
    }
