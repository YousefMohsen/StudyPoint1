...
public class EchoServer {
    ...
    private  OutputHandler1 outputHandler;
    ...
    private void handleConnection(Socket connection) throws IOException {
        ...
        type = outputHandler.findType(line);
        messege = outputHandler.findMessege(line);
        
        
        switch(type){
        
            case "UPPER":
                line = outputHandler.toUpper(messege);
                break;
            
            case "LOWER":
             line = outputHandler.toLower(messege);   
            break;
            case "REVERSE":
                line = outputHandler.reverse(messege);
                break;
